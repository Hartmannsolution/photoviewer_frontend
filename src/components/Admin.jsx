...
const EditTags = () => {
  ...
  /**
   * Get all tags from server and sort them alphabetically to view in selectbox.
   */
  const updateTags = () => {
  ...
  /**
   * 
   * @param {form event submit} evt 
   * Update current TAG on the server.
   */
  ...
  return (
    <>
     <div>{tags
        .map((t, idx) =>
          <span key={idx} id={t.name} style={{ color: 'blue' }} onClick={() => { setTag(t); }}>
            {idx > 0 && ', '}{t.name}
          </span>
        )}</div> 
        <br />
